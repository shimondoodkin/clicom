...
description: Use when you need to read screens, brief, recover, manage tokens, or hand off a wrapped CLI agent (Claude Code, Codex) via clicom. Applies in any phase of the round-loop where you act on an agent rather than judge an artifact. Mechanics only — judgment lives in the phase skills.
---
